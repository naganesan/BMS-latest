...
async function autoBookBMS(options = {}) {
  ...
  try {
    ...
    async function clickElementByBoundingBox(page, elementHandle) {
      if (!elementHandle) throw new Error('null element to clickElementByBoundingBox');
      try { await elementHandle.scrollIntoViewIfNeeded(); } catch (e) {}
    ...
    async function clickPayAndAcceptTerms(page) {
      ...
      async function clickElementByBoundingBoxLocal(page, handle) {
    ...
    try {
      // Helper to wait for either 'food-and-beverages' or 'order-summary' in URL within timeout
      async function waitForRedirectTarget(page, timeoutMs = 7000) {
        const deadline = Date.now() + timeoutMs;
        while (Date.now() < deadline) {
          try {
            const u = page.url();
            if (u && (u.includes('food-and-beverages') || u.includes('order-summary'))) return u;
          } catch (e) {}
          await sleep(250);
        }
      ...
      // Wait a short while for redirect / page change to settle (only until a target URL appears)
      const redirectedUrl = await waitForRedirectTarget(page, 7000);
      ...
      if (redirectedUrl && redirectedUrl.includes('food-and-beverages')) {
        ...
        // Wait for order-summary URL after skip click
        const deadline = Date.now() + 8000;
        let reachedOrder = false;
        while (Date.now() < deadline) {
          try {
            const u = page.url();
            if (u && u.includes('order-summary')) { reachedOrder = true; break; }
          } catch (e) {}
          await sleep(250);
        }
        if (!reachedOrder) {
          console.warn('After clicking Skip we did not detect order-summary URL within timeout; attempting to proceed anyway.');
        } else {
          console.log('Order-summary reached after Skip.');
        }

        // Now fill deemed-email & phone on order-summary
        if (EMAIL || PHONE) {
        ...
        // If neither known redirect detected, do a final attempt: if current page has order-summary path, handle it; else no-op
        const currentUrl = page.url();
        console.log('No explicit food/order redirect detected. Current URL:', currentUrl);
        if (currentUrl && currentUrl.includes('order-summary')) {
          if (EMAIL || PHONE) {
            const filled = await fillDeemedEmailAndPhone(page, EMAIL, PHONE);
            console.log('fillDeemedEmailAndPhone (fallback order-summary) ->', filled);
          }
        } else {
          console.log('Not on food-or-order-summary page. The script will continue and try payment selection if possible.');
        }
      }
    } catch (e) {
